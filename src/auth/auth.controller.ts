...
export class AuthController {
	...
	@HttpCode(200)
	@UsePipes(new ValidationPipe())
	@Post('auth/change-password/checkResetToken')
	async checkResetToken(
		@Body() dto: { token: string }
	) {
		const { token } = dto;

		const user = await this.authService.validateResetPasswordToken(token);
	...
	async changePassword(
		...
		const user = await this.authService.validateResetPasswordToken(resetToken);
